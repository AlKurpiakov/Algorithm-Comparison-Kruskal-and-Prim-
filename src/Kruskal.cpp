...
std::pair<weight, std::vector<std::pair<int, int>>> KruskalMst(const Graph graph){
    ...
    for (int i = 0; i < n-1; i++){
        if (!dsu.IsConnected(sorted_edges[i]._u, sorted_edges[i]._v)){
            total_weight += sorted_edges[i]._weight;
            dsu.UnionSet(sorted_edges[i]._u, sorted_edges[i]._v);
            mst_edges.emplace_back(sorted_edges[i]._u, sorted_edges[i]._v);
        }
